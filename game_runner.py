from board_item_test import BoardItem
import chess.engine
import time

# --- CONFIGURATION ---
STOCKFISH_PATH = "stockfish-windows-x86-64-avx2.exe"  # path to your Stockfish binary
ENGINE_TIME = 0.1        # seconds per move
TURN_DELAY = 0         # delay between turns
WHITE_SKILL = 10          # Stockfish skill for White
BLACK_SKILL = 15         # Stockfish skill for Black
SHOW_PATHS = True        # show planned paths
AUTO_PLAY = True         # True = Stockfish vs Stockfish, False = Human vs Computer

# --- SETUP BOARD ---
board_item = BoardItem()
board_item.display_state()
board_item.display_nodes()
board_item.display_board()

# --- INITIALIZE ENGINES ---
white_engine = chess.engine.SimpleEngine.popen_uci(STOCKFISH_PATH)
black_engine = chess.engine.SimpleEngine.popen_uci(STOCKFISH_PATH)
white_engine.configure({"Skill Level": WHITE_SKILL})
black_engine.configure({"Skill Level": BLACK_SKILL})

# --- GAME LOOP ---
turn = 0
while not board_item.chess_board.is_game_over():
    color = "White" if board_item.chess_board.turn == chess.WHITE else "Black"
    print(f"\n[{turn}] {color}'s turn")

    if AUTO_PLAY or color == "Black":  # Stockfish move
        engine = white_engine if board_item.chess_board.turn == chess.WHITE else black_engine
        result = engine.play(board_item.chess_board, chess.engine.Limit(time=ENGINE_TIME))
        move_uci = result.move.uci()
        print(f"{color} (Stockfish) plays: {move_uci}")
        promotion = None
        if len(move_uci) == 5:
            promotion = move_uci[-1]
            move_uci = move_uci[:4]
    else:
        # Human move
        move_uci = input("Enter your move (UCI notation like e2e4): ").strip()
        promotion = None
        if len(move_uci) == 5:  # e.g., e7e8q
            promotion = move_uci[-1]
            move_uci = move_uci[:4]

    # --- Plan and display move ---
    move_path = board_item.plan_path(move_uci, promotion=promotion)
    if SHOW_PATHS:
        print(f"{color} move path:")
        board_item.display_paths(move_path)
    print(f"Path to send to Arduino: {move_path}")

    # --- Execute move ---
    board_item.move_piece(move_uci, promotion=promotion)
    board_item.display_state()
    board_item.display_nodes()
    board_item.display_board()


# --- Pause for special moves ---
    #piece_moved = board_item.chess_board.piece_at(chess.parse_square(move_uci[2:4]))
    #is_castling = piece_moved and piece_moved.piece_type == chess.KING and abs(ord(move_uci[0]) - ord(move_uci[2])) > 1

    if promotion is not None: #or is_castling:
        print(f"{'Promotion' if promotion else 'Castling'} occurred! Pausing for 20 seconds...")
        time.sleep(20)

    # --- Delay for readability / visualization ---
    time.sleep(TURN_DELAY)
    turn += 1

# --- GAME OVER ---
print("\nGame over!")
print("Result:", board_item.chess_board.result())

# --- CLEANUP ---
white_engine.quit()
black_engine.quit()
