...
class BoardItem:
    ...
    def _node_to_square(self, node):
        """
        Convert a node-grid coordinate (r,c) into a python-chess square index.
        Returns None if the node doesn't map to a chessboard square.
        """
        r, c = node
        # node maps to state indices by dividing by 2
        if r % 2 != 0 or c % 2 != 0:
            return None
        sr, sc = r // 2, c // 2
        # chess board lives in state rows 1..8 and cols 2..9
        if 1 <= sr <= 8 and 2 <= sc <= 9:
            rank = 8 - sr
            file = sc - 2
            return chess.square(file, rank)
        return None

    def _square_to_node(self, square):
        """
        Convert a python-chess square index to node-grid coords.
        """
    ...
    def reset_board(self):
        """
        Plan and return movements that restore all pieces to a valid starting
        arrangement. Pieces of the same type/color can go to any matching
        starting square. If a desired target node is occupied, the blocker
        will be moved to a random free node first.

        Returns:
            path_seq (list): sequence of labeled path steps (same format as plan_path output).
            gcode (str): optional G-code produced from path_seq (useful to send directly to the gantry).
        Side effects:
            - Does NOT actually run the gantry; only plans moves.
            - Resets the logical python-chess board and internal visual boards at the end.
        """
        # ---- snapshot current physical piece positions (node grid) ----
        cur_positions = {}  # symbol -> list of node coords
        for r in range(self.node_rows):
            for c in range(self.node_cols):
                val = self.node_grid[r, c]
                if isinstance(val, str) and val not in ('.',) and not val.isdigit():
                    # treat single-character piece symbols only
                    cur_positions.setdefault(val, []).append((r, c))

        # ---- desired starting positions from a fresh chess.Board() ----
        start_board = chess.Board()  # standard initial chess starting pos
        desired_positions = {}  # symbol -> list of node coords for start squares
        for sq in chess.SQUARES:
            p = start_board.piece_at(sq)
            if p:
                sym = p.symbol()
                node = self._square_to_node(sq)
                desired_positions.setdefault(sym, []).append(node)

        # ---- helper: nearest matching piece selection (manhattan on nodes) ----
        def manhattan(a, b):
            return abs(a[0] - b[0]) + abs(a[1] - b[1])

        # available free node candidates for temporary moves (must avoid state/promo/capture nodes)
        free_nodes = [
            (r, c) for r in range(self.node_rows) for c in range(self.node_cols)
            if self.node_grid[r, c] == '.'
        ]

        # reserve set to avoid collisions when scheduling temp moves
        reserved_targets = set()

        # list of planned physical moves (from_node -> to_node)
        planned_moves = []

        # copy cur_positions so assignments remove used pieces
        cur_pos_copy = {k: v[:] for k, v in cur_positions.items()}

        # For each desired symbol and target node, pick nearest available current piece of same symbol.
        for sym, targets in desired_positions.items():
            # If there are more targets than existing pieces (shouldn't happen for normal chess), skip extras.
            for tgt in targets:
                # pick nearest available current piece of same symbol
                candidates = cur_pos_copy.get(sym, [])
                if not candidates:
                    # no matching physical piece found — skip; logical reset will set the state
                    continue
                # find nearest candidate
                nearest = min(candidates, key=lambda p: manhattan(p, tgt))
                candidates.remove(nearest)
                # If the target node is currently occupied by some other piece that is not the same moving piece,
                # schedule that occupant to be moved elsewhere first.
                occupant = self.node_grid[tgt[0], tgt[1]]
                if occupant != '.' and occupant != nearest and not (tgt in reserved_targets):
                    # pick a random free node that is not reserved and not equal to nearest
                    available_free = [n for n in free_nodes if n not in reserved_targets and n != nearest and n != tgt]
                    if not available_free:
                        # fallback: search for any node that isn't a state square (very unlikely)
                        raise RuntimeError("No available temporary free node to move blocker to.")
                    temp = random.choice(available_free)
                    reserved_targets.add(temp)
                    planned_moves.append((tgt, temp))  # move blocker from tgt -> temp
                # now schedule the real move: nearest -> tgt
                reserved_targets.add(tgt)
                planned_moves.append((nearest, tgt))

        # ---- convert planned_moves into path_seq using plan_path helper by mapping nodes to chess squares ----
        path_seq = []
        for (from_node, to_node) in planned_moves:
            from_sq = self._node_to_square(from_node)
            to_sq = self._node_to_square(to_node)
            # generate a UCI like "e2e4" even when moving occupiers into temporary nodes off board.
            # If either endpoint isn't on the 8x8 board, we create a synthetic UCI by mapping to the nearest valid
            # square (this is only to let plan_path compute node A*; plan_path uses node coords directly).
            if from_sq is None:
                # find a closest on-board square to the from_node for parsing (approximate)
                possible = []
                for sq in chess.SQUARES:
                    possible.append((manhattan(from_node, self._square_to_node(sq)), sq))
                from_sq = min(possible, key=lambda x: x[0])[1]
            if to_sq is None:
                possible = []
                for sq in chess.SQUARES:
                    possible.append((manhattan(to_node, self._square_to_node(sq)), sq))
                to_sq = min(possible, key=lambda x: x[0])[1]
            uci = chess.square_name(from_sq) + chess.square_name(to_sq)
            # plan path between the two node coordinates (plan_path will map the squares to the node grid)
            sub_paths = self.plan_path(uci)
            # plan_path returns a list (e.g. [('move', [...])]); we only want the path most relevant;
            # simply extend the path_seq with the returned steps but relabel them as 'reset_move'
            for step, p in sub_paths:
                # flatten any None to []
                if p is None:
                    p = []
                path_seq.append(('reset_move', p))

        # After planning, sanitize entries and return something usable even if empty
        # collapse consecutive empty paths removed in plan_path call already
        # produce G-code
        gcode = self.generate_gcode(path_seq)

        # ---- finally, reset the logical chess board and update visuals ----
        self.chess_board.reset()
        self.captured_white = []
        self.captured_black = []
        # restore promotion lists to default
        self.white_promos = ['B','N','R','Q','Q','Q','Q','B','N','R']
        self.black_promos = ['b','n','r','q','q','q','q','b','n','r']
        # regenerate state and node visuals to reflect canonical starting layout
        self.update_from_chess()

        return path_seq, gcode
